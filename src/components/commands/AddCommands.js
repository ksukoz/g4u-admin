...
import { getCountries, getCities, getRegions } from '../../actions/locationActions';
import { getPlayersByName } from '../../actions/playerActions';
import { addCommand, getCommandsByName, clearCommands } from '../../actions/commandsActions';
	...
	render() {
		...
		if (countries !== null && countries !== undefined) {
			countriesList = countries.map((country) => (
				<MenuItem key={country.id} value={JSON.stringify({ id: country.id, iso: country.iso })}>
					{country.name}
				</MenuItem>
			));
		}
