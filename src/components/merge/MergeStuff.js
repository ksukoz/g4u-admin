...
import { getStuffMembersByName, mergeStuff, clearStuff } from '../../actions/stuffActions';
	...
	render() {
		...
		if (members !== null) {
			memberList = members.map((member) => (
				<TableRow key={`member${member.id}`} data-id={member.id} data-name="member" onClick={this.onClick}>
		...
		if (usersForMerge !== null) {
			userArr = usersForMerge.map((user) => (
				<TableRow key={user.id} data-id={user.id} data-name="user" onClick={this.onClick}>
					<TableCell component="th" scope="row" className={classes.cell}>
						<Radio
