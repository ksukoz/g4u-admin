export const GET_ERRORS = 'GET_ERRORS';
export const GET_MESSAGES = 'GET_MESSAGES';
export const SET_LANGUAGE = 'SET_LANGUAGE';
export const GET_COUNTRIES = 'GET_COUNTRIES';
export const GET_ALL_COUNTRIES = 'GET_ALL_COUNTRIES';
export const GET_REGIONS = 'GET_REGIONS';
export const GET_CITIES = 'GET_CITIES';
export const OPEN_NAV = 'OPEN_NAV';
export const CLOSE_NAV = 'CLOSE_NAV';
export const SET_ACTIVE_LINK = 'SET_ACTIVE_LINK';

export const GET_USER = 'GET_USER';
export const SET_USER = 'SET_USER';
export const LOGOUT_USER = 'LOGOUT_USER';

export const GET_LEAGUES = 'GET_LEAGUES';
export const GET_SUB_LEAGUES = 'GET_SUB_LEAGUES';
export const GET_CURRENT_LEAGUE = 'GET_CURRENT_LEAGUE';

export const GET_STADIUMS = 'GET_STADIUMS';
export const GET_STADIUMS_BY_NAME = 'GET_STADIUMS_BY_NAME';
export const CLEAR_STADIUMS = 'CLEAR_STADIUMS';

export const GET_STUFF_TYPES = 'GET_STUFF_TYPES';
export const GET_STUFF_MEMBERS = 'GET_STUFF_MEMBERS';
export const GET_STUFF_MEMBERS_BY_NAME = 'GET_STUFF_MEMBERS_BY_NAME';
export const GET_STUFF_FOR_APP = 'GET_STUFF_FOR_APP';
export const MERGE_STUFF = 'MERGE_STUFF';
export const CLEAR_STUFF = 'CLEAR_STUFF';

export const GET_USERS = 'GET_USERS';
export const GET_USERS_BY_NAME = 'GET_USERS_BY_NAME';

export const GET_TOURNAMENTS = 'GET_TOURNAMENTS';
export const GET_SEASONS = 'GET_SEASONS';
export const GET_SUBTOURNAMENTS = 'GET_SUBTOURNAMENTS';
export const GET_TOUR_COMMANDS = 'GET_TOUR_COMMANDS';
export const GET_TOUR_GAMES = 'GET_TOUR_GAMES';
export const GET_GAME_BY_ID = 'GET_GAME_BY_ID';
export const GET_AUTO_GAMES = 'GET_AUTO_GAMES';
export const CLEAR_AUTO_GAMES = 'CLEAR_AUTO_GAMES';
export const GET_GAMES_BY_NAME = 'GET_GAMES_BY_NAME';
export const GET_APPOINTS = 'GET_APPOINTS';
export const GET_GAME_APPOINTS = 'GET_GAME_APPOINTS';
export const GET_DATELESS_GAMES = 'GET_DATELESS_GAMES';
export const GET_GAME_PLAYERS = 'GET_GAME_PLAYERS';
export const CLEAR_TOUR_GAMES = 'CLEAR_TOUR_GAMES';

export const GET_POSITION = 'GET_POSITION';
export const GET_PLAYERS = 'GET_PLAYERS';
export const GET_PLAYERS_BY_NAME = 'GET_PLAYERS_BY_NAME';
export const GET_PLAYERS_REQUESTS = 'GET_PLAYERS_REQUESTS';
export const GET_REQUESTED_PLAYER = 'GET_REQUESTED_PLAYER';
export const GET_PLAYERS_BY_NAME_AND_COMMAND = 'GET_PLAYERS_BY_NAME_AND_COMMAND';
export const MERGE_PLAYER = 'MERGE_PLAYER';
export const GET_MERGING = 'GET_MERGING';
export const CLEAR_PLAYERS = 'CLEAR_PLAYERS';
export const CLEAR_COMMAND_PLAYERS = 'CLEAR_COMMAND_PLAYERS';

export const GET_COMMANDS_BY_NAME = 'GET_COMMANDS_BY_NAME';
export const GET_COMMANDS = 'GET_COMMANDS';
export const GET_COMMAND_BY_ID = 'GET_COMMAND_BY_ID';
export const CLEAR_COMMANDS = 'CLEAR_COMMANDS';
