...
class MergePlayers extends Component {
	...
	render() {
		...
		const { membersForMerge } = this.props.players;
		const { usersForMerge } = this.props.users;
		let memberList;
		let userArr;
		if (membersForMerge !== null) {
			memberList = membersForMerge.map((member) => (
				<TableRow key={`member${member.plId}`} onClick={this.onClick('member', member.plId)}>
					<TableCell className={classes.cell}>
						<Radio
							checked={this.state.playersId === member.plId}
							onChange={this.onRadioChangeHandler}
							value={member.plId}
							name="member"
							aria-label={member.plId}
							classes={{
								root: classes.root,
		...
		if (usersForMerge !== null) {
			userArr = usersForMerge.map((user) => (
				<TableRow key={user.id} onClick={this.onClick('user', user.id)}>
					<TableCell component="th" scope="row" className={classes.cell}>
						<Radio
		...
		return (
			<div className={classes.container}>
				...
				<form onSubmit={this.onSubmit}>
					<Button className={classes.submit} type="submit" size="large" variant="contained">
